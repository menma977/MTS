...
Route::middleware(['online', 'auth'])->group(static function () {
  ...
  Route::group(['prefix' => 'tree', 'as' => 'tree.'], static function () {
    Route::get('/', 'TreeController@index')->name('index')->middleware('auth', 'role:0');
    Route::post('/store', 'TreeController@store')->name('store')->middleware('auth', 'role:0');
    Route::get('/show/{id}', 'TreeController@show')->name('show')->middleware('auth', 'role:0');
    Route::get('/harvest/{id}', 'TreeController@harvest')->name('harvest')->middleware('auth', 'role:0');
    Route::get('/{id}/update/{status}', 'TreeController@update')->name('update')->middleware('auth', 'role:0');
    Route::get('/delete/{id}', 'TreeController@destroy')->name('delete')->middleware('auth', 'role:0');
    Route::post('/qr', 'TreeController@QRCodeList')->name('QRCodeList')->middleware('auth', 'role:0');
    Route::get('/qr/{id}', 'TreeController@QRCode')->name('QRCode')->middleware('auth', 'role:0');
  });
